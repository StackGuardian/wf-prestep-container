...
if [ -z "$SG_API_TOKEN" ]; then
  echo "ERROR: SG_API_TOKEN environment variable is not set" >&2
  exit 1
fi
...
AUTH_HEADER="Authorization: apikey $SG_API_TOKEN"
...
#   -H "Content-Type: application/json" \
#   -d "$UPDATED_CONFIG" \
#   "$API_BASE_URL/orgs/$SG_ORG_ID/workflows/$WORKFLOW_ID")
